...
$aa = mysqli_query($GLOBALS["___mysqli_ston"], "update pesanan SET `read`='1' where id=$_GET[id]") or die(mysqli_error($GLOBALS["___mysqli_ston"]));
// if(!empty($_GET)){
// if($_GET['act'] == 'delete'){
...
<div class="container">
	<?php
	$q = mysqli_query($GLOBALS["___mysqli_ston"], "select*from pesanan where id='$_GET[id]'");
	$data = mysqli_fetch_object($q);
	$ongkir = $data->ongkir;
	$kota = $data->kota;
	$dataPembayaran = mysqli_query($GLOBALS["___mysqli_ston"], "select * from pembayaran where id_pesanan='$data->id' and status='verified'") or die(mysqli_error($GLOBALS["___mysqli_ston"]));
	$totalPembayaran = 0;
	while ($d = mysqli_fetch_array($dataPembayaran)) {
		$totalPembayaran += $d['total'];
	}

	$q1 = mysqli_query($GLOBALS["___mysqli_ston"], "select*from detail_pesanan where pesanan_id='$data->id'");
	$totalBayar = 0;
	...
	<div class="row col-md-12">
		...
		<table class="table table-striped table-hove">
			...
			<tbody>
				<?php
				$q = mysqli_query($GLOBALS["___mysqli_ston"], "select*from detail_pesanan where pesanan_id='$_GET[id]'");
				$total = 0;
		...
		<p class="print"><strong>Ctrl + p </strong><i>*untuk mencetak invoice pembayaran*</i></p>
	</div>
</div> <!-- /container -->
