...
?>
		...
		<?php
			if(!empty($_POST['form-order'])){
				...
				$kt = mysqli_fetch_object(mysqli_query($GLOBALS["___mysqli_ston"], "SELECT *FROM kota where id='$kota'"));
				if($difference->days >= 3){
					
					$q = mysqli_query($GLOBALS["___mysqli_ston"], "insert into pesanan Values(NULL,'$tanggal_pesan','$tanggal_digunakan','$_SESSION[iam_user]','$nama','$alamat','$kt->nama','$kt->ongkir','$telephone','0','belum lunas')");
					if($q){

						$last = mysqli_fetch_array(mysqli_query($GLOBALS["___mysqli_ston"], "SELECT *FROM pesanan order by id DESC limit 1"));
						...
						foreach($cart as $id => $qty){
							...
							if(!empty($product)){
								...
								$ins = mysqli_query($GLOBALS["___mysqli_ston"], "insert into detail_pesanan Values(NULL,'$product[id]','$qty','$last[id]')");
				...
				<hr>
				 <form action="" method="post" enctype="multipart/form-data">
						...
						<select name="kota" required class="form-control">
							<?php
								$kota = mysqli_query($GLOBALS["___mysqli_ston"], "select*from kota");
