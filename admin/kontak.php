<?php 
	...
	if(!empty($_GET)){
		if($_GET['act'] == 'delete'){
			...
			$q = mysqli_query($GLOBALS["___mysqli_ston"], "delete from kontak WHERE id='$_GET[id]'");
	...
	if(!empty($_GET['act']) && $_GET['act'] == 'create'){
		if(!empty($_POST)){
			...
			$q = mysqli_query($GLOBALS["___mysqli_ston"], "insert into kontak Values(NULL,'$nama','$email','$subjek','$pesan')");
			if($q){ alert("Success"); redir("kontak.php"); }
		}
	}
	if(!empty($_GET['act']) && $_GET['act'] == 'edit'){
		if(!empty($_POST)){
			...
			$q = mysqli_query($GLOBALS["___mysqli_ston"], "update kontak SET nama='$nama',email='$email',subjek='$subjek',pesan='$pesan' where id=$_GET[id]") or die(mysqli_error($GLOBALS["___mysqli_ston"]));
			if($q){ alert("Success"); redir("kontak.php"); }
		}
	}
...
?> 
	...
	<div class="container">
		<?php
			$q = mysqli_query($GLOBALS["___mysqli_ston"], "select*from kontak");
			$j = mysqli_num_rows($q);
		?>
		...
		<?php
			if(!empty($_GET)){
				...
				if($_GET['act'] == 'edit'){
					$data = mysqli_fetch_object(mysqli_query($GLOBALS["___mysqli_ston"], "select*from kontak where id='$_GET[id]'"));
				?>
					<div class="row col-md-6">
