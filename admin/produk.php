<?php 
	...
	if(!empty($_GET)){
		if($_GET['act'] == 'delete'){
			...
			$q = mysqli_query($GLOBALS["___mysqli_ston"], "delete from produk WHERE id='$_GET[id]'");
	...
	if(!empty($_GET['act']) && $_GET['act'] == 'create'){
		if(!empty($_POST)){
			...
			$q = mysqli_query($GLOBALS["___mysqli_ston"], "insert into produk Values(NULL,'$nama','$deskripsi','$gambar','$harga','$kategori_produk_id')");
			if($q){
	...
	if(!empty($_GET['act']) && $_GET['act'] == 'edit'){
		if(!empty($_POST)){ 
			...
			$q = mysqli_query($GLOBALS["___mysqli_ston"], "update produk SET nama='$nama',deskripsi='$deskripsi', gambar='$gambar', harga='$harga', kategori_produk_id='$kategori_produk_id' where id=$_GET[id]") or die(mysqli_error($GLOBALS["___mysqli_ston"]));
			if($q){
				$upload = move_uploaded_file($_FILES['gambar']['tmp_name'], '../uploads/'.$gambar);
				if($upload){ alert("Success"); redir("produk.php"); }
			}
...
?> 
	...
	<div class="container">
		<?php
			$q = mysqli_query($GLOBALS["___mysqli_ston"], "select*from produk");
			$j = mysqli_num_rows($q);
		?>
		...
		<?php
			if(!empty($_GET)){
				...
				?>
					...
					<form action="" method="post" enctype="multipart/form-data">
						...
						<select name="kategori_produk_id" required class="form-control"> 
							<?php
								$katpro = mysqli_query($GLOBALS["___mysqli_ston"], "select*from kategori_produk");
				...
				if($_GET['act'] == 'edit'){
					$data = mysqli_fetch_object(mysqli_query($GLOBALS["___mysqli_ston"], "select*from produk where id='$_GET[id]'"));
				?>
					<div class="row col-md-6">
					<form action="produk.php?act=edit&&id=<?php echo $_GET['id'] ?>" method="post" enctype="multipart/form-data">
						...
						<select name="kategori_produk_id" required class="form-control"> 
						<?php
								$katpro = mysqli_query($GLOBALS["___mysqli_ston"], "select*from kategori_produk where id='$data->kategori_produk_id'");
