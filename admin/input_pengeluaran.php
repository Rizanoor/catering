<?php
	...
	if(!empty($_GET)){
		if($_GET['act'] == 'delete'){
			...
			$q = mysqli_query($GLOBALS["___mysqli_ston"], "delete from laporan WHERE id_pengeluaran='$_GET[id]'");
	...
	if(!empty($_GET['act']) && $_GET['act'] == 'create'){
		if(!empty($_POST)){
			extract($_POST);
			$q = mysqli_query($GLOBALS["___mysqli_ston"], "insert into laporan Values(NULL,'$nama_barang','$Tanggal_pengeluaran','$harga','$jumlah',$harga*$jumlah)");
			if($q){ alert("Success"); redir("input_pengeluaran.php"); }
		}
	}
	if(!empty($_GET['act']) && $_GET['act'] == 'edit'){
		if(!empty($_POST)){
			...
			$q = mysqli_query($GLOBALS["___mysqli_ston"], "update laporan SET nama_barang='$nama_barang',harga='$harga',jumlah='$jumlah',total='$total' where id_pengeluaran=$_GET[id]") or die(mysqli_error($GLOBALS["___mysqli_ston"]));
			if($q){ alert("Success"); redir("input_pengeluaran.php"); }
		}
	}
...
?>
	...
	<div class="container">
		<?php
			$q = mysqli_query($GLOBALS["___mysqli_ston"], "select*from laporan");
			$j = mysqli_num_rows($q);
		?>
		...
		<?php
			if(!empty($_GET)){
				...
				if($_GET['act'] == 'edit'){
					$data = mysqli_fetch_object(mysqli_query($GLOBALS["___mysqli_ston"], "select*from laporan where id_pengeluaran='$_GET[id]'"));
				?>
					<div class="row col-md-6">
