<?php 
	...
	if(!empty($_GET)){
		if($_GET['act'] == 'delete'){
			...
			$q = mysqli_query($GLOBALS["___mysqli_ston"], "delete from kota WHERE id='$_GET[id]'");
	...
	if(!empty($_GET['act']) && $_GET['act'] == 'create'){
		if(!empty($_POST)){
			extract($_POST);
			$q = mysqli_query($GLOBALS["___mysqli_ston"], "insert into kota Values(NULL,'$nama','$ongkir')");
			if($q){ 
	...
	if(!empty($_GET['act']) && $_GET['act'] == 'edit'){
		if(!empty($_POST)){ 
			extract($_POST);
			$q = mysqli_query($GLOBALS["___mysqli_ston"], "update kota SET nama='$nama',ongkir='$ongkir' where id=$_GET[id]") or die(mysqli_error($GLOBALS["___mysqli_ston"]));
			if($q){
				alert("Success"); redir("kota.php");
			}
		}
...
?> 
	...
	<div class="container">
		<?php
			$q = mysqli_query($GLOBALS["___mysqli_ston"], "select*from kota");
			$j = mysqli_num_rows($q);
		?>
		...
		<?php
			if(!empty($_GET)){
				...
				if($_GET['act'] == 'edit'){
					$data = mysqli_fetch_object(mysqli_query($GLOBALS["___mysqli_ston"], "select*from kota where id='$_GET[id]'"));
				?>
					<div class="row col-md-6">
