...
<html lang="en">
  ...
  $qpesanan = mysqli_query($GLOBALS["___mysqli_ston"], "select*from pesanan where `read`='0'") or die (mysqli_error($GLOBALS["___mysqli_ston"]));
  $totalUnRead = mysqli_num_rows($qpesanan);
  // TOTAL PEMBAYARAN YANG BELUM DI VERIFIKASI
  $qPembayaran = mysqli_query($GLOBALS["___mysqli_ston"], "select*from pembayaran where `status`='pending'") or die (mysqli_error($GLOBALS["___mysqli_ston"]));
    ...
    <nav class="navbar navbar-default navbar-fixed-top navbar-blue">
      <div class="container">
        <div class="navbar-header">
          ...
          <a class="navbar-brand" href="#">Love Me Baked</a>
        </div>
