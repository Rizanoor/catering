...
$q = mysqli_query($GLOBALS["___mysqli_ston"], "select*from pesanan where user_id='$_SESSION[iam_user]'");
$j = mysqli_num_rows($q);
?>
					...
					<tbody>
						<?php while ($data = mysqli_fetch_object($q)) { ?>
							<tr <?php if ($data->read == 0) {
								...
								?>
								<td><?php echo $data->nama ?><br><a href="invoice.php?id=<?php echo $data->id ?>">Invoice pembayaran</a></td>
								<td><?php echo substr($data->tanggal_pesan, 0, 10) ?></td>
								<td><?php echo $data->tanggal_digunakan ?></td>
								<td><?php echo $data->telephone ?></td>
								<td><?php echo $data->alamat ?></td>
								<!-- <td><a class="btn btn-sm btn-warning lihat" href="invoice.php?id=<?php echo $data->id ?>">Lihat</a> -->
								</td>
								<!--td>  
